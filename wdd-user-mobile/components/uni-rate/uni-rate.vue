<template>
	<view>
		<view ref="uni-rate" class="uni-rate">
			<view class="uni-rate__icon" :style="{ 'margin-right': margin + 'rpx' }" v-for="(star, index) in stars" :key="index"
...
<script>
	 ...
	 * @property {Number} 	margin 						星星的间距，单位 rpx
	...
	export default {
		...
		methods: {
			...
			_getRateCount(clientX) {
				...
				const index = parseInt(rateMoveRange / (this.size / 2 + this.margin))
				const range = parseInt(rateMoveRange - ((this.size / 2 + this.margin) * index))
				let value = 0
