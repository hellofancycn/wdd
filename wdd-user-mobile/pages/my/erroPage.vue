						...
						<view class="question-contain">
							...
							<view class="question-show">
								<view class="exam-qustion-content">
									<renderHtml
										:content="`${questionItem.questionJson.title}`" />
								</view>

								<view v-if="questionItem.questionType === 1" class="single-choice">
									<radio-group class="question-radio-group">
										<label class="choice-label"
											v-for="qItem in questionItem.questionJson.questionItemFrames"
											:key="qItem.key">
											<radio class="choice-radio" :checked="qItem.key === questionItem.contentKey"
									...
									<checkbox-group class="question-checkbox-group">
										<label class="checkbox-label"
											...
											<checkbox class="choice-checkbox"
												:checked="questionItem.contentArrayKey.includes(qItem.key)"
												style="transform:scale(0.7)" color="#68bfd6" />
											<view class="exam-qustion-content"
												v-html="`${qItem.prefix}. ${qItem.content}`" />
										</label>
									...
									<radio-group class="question-true-false-group">
										<label class="true-false-label"
											...
											<radio class="true-false-radio"
												:checked="qItem.key === questionItem.contentKey"
												style="transform:scale(0.7)" color="#68bfd6" />
								...
								<view class="exam-list-contain">
									...
									<view class="exam-list-contain-content"
										...
										{{questionItem.questionJson.correctPrefix}}
									</view>
									<view v-else-if="questionItem.questionType === 3" class="exam-list-contain-content">
										{{questionItem.questionJson.correctContent}}
									</view>
									...
									<view v-else-if="questionItem.questionType === 5" class="exam-list-contain-content"
										v-html="questionItem.questionJson.correct"></view>
								</view>
								<view class="exam-list-contain">
					...
					<view v-for="(tItem,tIndex) in paper.examPaperDoTitleList" :key="tIndex">
						...
						<view class="anchor-quesiton-tag-list">
							<uni-tag class="tag" :text="qItem.itemOrder.toString()"
								...
								:type="correctPrefix[qItem.itemOrder-1].doRight === null ? 'warning' : answer.questionAnswerFrameList[qItem.itemOrder-1].doRight ? 'success' : 'error'"
								:key="qItem.itemOrder" />
						</view>
					</view>
				</scroll-view>
...
<script>
	...
	export default {
		...
		onLoad(options) {
			let _this = this
			practiceApi.errorPage(this.queryParam).then(res => {
				console.log(res, '错题')
				if (res.code === 1) {
					this.questionList = res.response.list
