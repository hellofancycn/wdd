				...
				<view class="question-dialog">
					<view class="question-dialog-content" v-if="!currentAnchor.submit">
						...
						<view v-if="currentAnchor.question.questionType === 4" class="gap-filling">
							<view v-for="(item,index) in currentAnchor.question.questionItemFrames" :key="index">
								<view class="gap-filling-widget">
									<view class="prefix">{{item.prefix}}</view>
									<view class="editor-widget">
										<view class="uni-textarea">
											<input class="uni-input filling-input" :data-itemIndex="index"
...
<script>
	...
	export default {
		...
		onLoad(options) {
			this.courseWareId = options.cId
			this.uId = options.uId
			courseWareApi.select(this.courseWareId).then(res => {
				if (res.code === 1) {
					let response = aesDecryptStr(res.response)
					this.courseWare = response
					// #ifdef H5
					this.pdfUrl = `/hybrid/html/web/viewer.html?name=${response.name}&file=${response.previewPath}`
					// #endif
					// #ifdef APP-PLUS
					this.pdfUrl =
						`${Web_URL}/hybrid/html/web/viewer.html?name=${response.name}&file=${response.previewPath}`
					// #endif
					// #ifdef MP-WEIXIN
					this.pdfUrl =
						`${Web_URL}/hybrid/html/web/viewer.html?name=${response.name}&file=${response.previewPath}`
		...
		methods: {
							...
							uni.showModal({
								...
								success: function(reConfirm) {
									if (reConfirm.confirm) {
										lastPageAction(getCurrentPages(), 'pages/train/detail', vm => {
											vm.reLoadDate()
										})
										uni.navigateBack()
									}
			...
			watchQuestion() {
				...
				for (let i = 0; i < questionCount; i++) {
					...
					if (item.anchorSecond === this.timeUpdate.second) {
						...
						// #ifdef APP-PLUS
						this.$scope.$getAppWebview().children()[0].setStyle({
							top: '0px',
							left: '0px',
							width: '1%',
							height: '1%'
			...
			inputChange(e) {
				...
				this.currentAnchor.answer.contentArray[itemIndex] = e.detail.value
			},
			textareaChange(e) {
