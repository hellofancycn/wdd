...
<script>
	...
	import trainExamPaperApi from '@/api/trainExamPaper'
	...
	export default {
		...
		onLoad(options) {
			let _this = this
			_this.cacheKey = `wdd-train-exam-answer-${options.pId}`
			_this.uId = options.uId
			trainExamPaperApi.select(options.uId, options.pId).then(res => {
				if (res.code === 1) {
		...
		methods: {
			...
			formSubmit() {
				...
				trainExamPaperApi.submit(this.uId, aesEncryptStr(this.answer)).then(re => {
					if (re.code === 1) {
						uni.removeStorageSync(_this.cacheKey);
						uni.showModal({
