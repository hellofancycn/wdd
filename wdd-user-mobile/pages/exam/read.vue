						...
						<view class="question-contain">
							...
							<view class="question-show">
								...
								<view class="answer-item-result">
									...
									<uni-tag
										v-if="answer.questionAnswerFrameList[questionItem.itemOrder-1].doRight === null"
					...
					<view v-for="(tItem,tIndex) in paper.examPaperDoTitleList" :key="tIndex">
						...
						<view class="anchor-quesiton-tag-list">
							<uni-tag class="tag" :text="qItem.itemOrder.toString()"
								...
								:type="answer.questionAnswerFrameList[qItem.itemOrder-1].doRight === null ? 'warning' : answer.questionAnswerFrameList[qItem.itemOrder-1].doRight ? 'success' : 'error'"
								:key="qItem.itemOrder" />
						</view>
					</view>
				</scroll-view>
...
<script>
	...
	export default {
		...
		onLoad(options) {
			let _this = this
			examPaperAnswerApi.read(options.id).then(res => {
				if (res.code === 1) {
