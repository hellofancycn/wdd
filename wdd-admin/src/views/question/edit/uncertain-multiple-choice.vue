            ...
            <el-form-item label="选项：" prop="items" class="editor-form-item" required>
              <el-form-item :label="item.prefix" :key="index" v-for="(item,index) in form.items"
                ...
                <div class="question-list-item">
                  ...
                  <!-- <el-input-number v-model="item.score" :precision="1" :step="1"
                                   :max="100" placeholder="分数"/> -->
...
import {uncertainMultipleChoiceSelect, uncertainMultipleChoiceEdit} from '@/api/question'
...
export default {
  ...
  data() {
    ...
    return {
      form: {
        ...
        title: '',
        items: [
          {id: null, prefix: 'A', content: '', score: null},
          {id: null, prefix: 'B', content: '', score: null},
          {id: null, prefix: 'C', content: '', score: null},
          {id: null, prefix: 'D', content: '', score: null}
        ],
        analyze: '',
        correct: null,
        correctArray: [],
  ...
  created() {
    ...
    if (id && parseInt(id) !== 0) {
      ...
      uncertainMultipleChoiceSelect(id).then(re => {
  ...
  methods: {
    ...
    questionItemAdd() {
      ...
      items.push({id: null, prefix: newLastPrefix, content: '', score: null})
    },
    submitForm() {
      this.$refs.form.validate((valid) => {
        if (valid) {
          this.formLoading = true
          uncertainMultipleChoiceEdit(this.form).then(re => {
            if (re.code === 1) {
