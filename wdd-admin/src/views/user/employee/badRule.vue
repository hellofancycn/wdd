...
import { userExtendViolationPage, userExtendViolationInsert, userExtendViolationDelete, userExtendViolationUpdate } from '@/api/user'
  ...
  methods: {
    ...
    userDelete(row) {
      this.$confirm('将执行删除操作, 是否继续?', '提示', {
        ...
        userExtendViolationDelete(row.id).then(re => {
          if (re.code === 1) {
            this.search()
