<template>
  <div class="app-container extract-contain">
        ...
        <el-form :model="form" ref="form" label-width="100px" v-loading="formLoading" :rules="rules">
          ...
          <el-form-item label="考试员工：" class="tree-select-contain"
            ...
            <div class="paper-user">
              <el-tag v-for="user in form.buildConfig.examPaperUserSelectList" :key="user.id" @close="removeUser(user)"
              ...
              <el-upload accept=".xlsx" action="/api/exam/paper/build/employee/import" :show-file-list="false"
                ...
                <el-button type="success" size="small" class="link-left wdd-button-mini">导入员工
                </el-button>
              </el-upload>
            </div>
          </el-form-item>
          <el-form-item label="考试口令：" prop="secret" required>
          ...
          <el-form-item label="考试时间：" v-if="form.rangeType !== null && form.rangeType === 2"
            ...
            <el-date-picker v-model="form.limitDateTime" value-format="YYYY-MM-DD HH:mm:ss" type="datetimerange"
              range-separator="至" start-placeholder="考试开始时间" end-placeholder="考试结束时间" :disabled-date="disabledDate">
            </el-date-picker>
          </el-form-item>
          <el-form-item>
            ...
            <el-tooltip content="离开试卷超过多次会自动提交试卷">
              <div class="exam-paper-cheat-div">
                ...
                <el-input-number v-model="form.buildConfig.maxCheatCount" placeholder="防作弊次数" :step="1" :max="99"
                  :min="1" size="small" v-if="form.buildConfig.cheat" class="max-cheat-count" />
              </div>
            </el-tooltip>
          ...
          <el-form-item :key="index" :label="`标题${index + 1}：`" v-for="(titleItem, index) in form.titleItems"
            ...
            <div class="exam-paper-title">
              ...
              <el-button type="success" class="link-left" style="margin-left: 20px" @click="addQuestion(titleItem)"
                v-if="action !== 1">选择题目
              </el-button>
              ...
              <el-button type="danger" class="link-left" @click="removeTitleItem(index)" v-if="action !== 1">
                删除
              </el-button>
            </div>
              ...
              <draggable class="paper-question-contain" item-key="index" group="titleItemList" handle=".drag"
                ...
                <template #item="{ index, element }">
                  <div class="paper-question-contain-item">
                    ...
                    <div class="action">
                      <!-- <el-input-number v-model="element.score" :precision="1" :step="1" :max="100" :min="0.1"
                        @change="sumScoreFuc(true)"></el-input-number> -->
                      <el-button type="success" style="width: 100%;margin-top: 10px"
                        @click="editQuestion(element.questionType, element.id, index, titleItem)" v-if="action !== 1">编辑题目
                      </el-button>
                      <el-button type="danger" style="width: 100%;margin: 10px 0px 0px 0px"
                        @click="removeQuestion(titleItem, index)" v-if="action !== 1">删除
                      </el-button>
                    </div>
                  </div>
                </template>
          ...
          <el-form-item label="单题分数：" prop="oneScore" >
            <el-input-number v-model="form.oneScore" :precision="1" :step="1" :min="0.1"></el-input-number>
          </el-form-item>
          ...
          <el-form-item label="考试时长：" prop="suggestTime" required>
            <el-input-number v-model="form.suggestTime" placeholder="分钟" :step="1" :max="600"
          ...
          <el-form-item v-if="action !== 1">
            <el-button type="primary" @click="submitForm"
              v-has-perm="['exam:paper:build:extract:create', 'exam:paper:build:random:update']">提交
            </el-button>
    ...
    <div class="extract-left">
      <el-card header="逢查必考编辑：">
        <el-form :model="form.buildConfig.extract" ref="extractForm" label-width="110px" v-loading="extractFormLoading">
          <el-form-item label="题目分类：">
            <tree-select v-model="form.buildConfig.extract.questionArchiveIdList" :data="questionArchiveTree"
              :clearable="true" placeholder="题目分类" style="width: 100%" />
          </el-form-item>
          <el-form-item label="单选题：" prop="singleChoice">
            <el-input-number v-model="form.buildConfig.extract.singleChoice" :step="1" :max="100"
              :min="0"></el-input-number>
          </el-form-item>
          <el-form-item label="多选题：" prop="multipleChoice">
            <el-input-number v-model="form.buildConfig.extract.multipleChoice" :step="1" :max="100"
              :min="0"></el-input-number>
          </el-form-item>
          ...
          <el-form-item label="判断题：" prop="trueFalse">
            <el-input-number v-model="form.buildConfig.extract.trueFalse" :step="1" :max="100"
              :min="0"></el-input-number>
          </el-form-item>
          <el-form-item label="填空题：" prop="gapFilling">
            <el-input-number v-model="form.buildConfig.extract.gapFilling" :step="1" :max="100"
              :min="0"></el-input-number>
          </el-form-item>
          <el-form-item label="简答题：" prop="shortAnswer">
            <el-input-number v-model="form.buildConfig.extract.shortAnswer" :step="1" :max="100"
              :min="0"></el-input-number>
          </el-form-item>
          <el-form-item label="难度：" prop="difficult">
            <el-rate v-model="form.buildConfig.extract.difficult" class="question-item-rate" show-text :max="3"
              :texts="['简单', '较难', '困难']" clearable></el-rate>
          </el-form-item>
          <el-form-item style="text-align: center" v-if="action !== 1">
    ...
    <el-dialog v-model="applyPage.showDialog" width="90%">
      <el-form :model="applyPage.queryParam" ref="queryForm" :inline="true">
        ...
        <el-form-item label="报名分类：">
          <tree-select v-model="applyPage.queryParam.applyArchiveIdList" :data="applyArchiveTree" :clearable="true"
            placeholder="报名分类" />
        </el-form-item>
        <el-form-item>
      ...
      <el-table ref="applyMultipleTable" v-loading="applyPage.listLoading" :data="applyPage.tableData"
        @current-change="applyHandleChange" border fit highlight-current-row style="width: 100%">
      ...
      <pagination v-show="applyPage.total > 0" :total="applyPage.total" v-model:page="applyPage.queryParam.pageIndex"
        v-model:limit="applyPage.queryParam.pageSize" @pagination="applySearch" :autoScroll="false" />
    </el-dialog>
      ...
      <el-form :model="questionPage.queryParam" ref="questionQueryForm" :inline="true">
        ...
        <el-form-item label="题目分类：">
          <tree-select v-model="questionPage.queryParam.questionArchiveIdList" :data="questionArchiveTree"
            :clearable="true" placeholder="题目分类" />
        </el-form-item>
        <el-form-item>
      ...
      <el-table v-loading="questionPage.listLoading" ref="questionMultipleTable" :data="questionPage.tableData"
        ...
        <el-table-column type="expand">
          ...
          <template #default="{ row }">
            <QuestionShow :qType="row.question.questionType" :question="row.question" class="table-question-show" />
          </template>
        </el-table-column>
...
import QuestionShow from '@/views/exam/build/components/Show.vue'
import { select, edit, checkExtract } from '@/api/examPaperBuild'
...
export default {
  ...
  methods: {
    ...
    sumScoreFuc(resetPassScore) {
      ...
      console.log(this.form.titleItems, '数据')
    },
    submitForm() {
          ...
          edit(this.form).then(re => {
            if (re.code === 1) {
              ...
              tagsView.delCurrentView(this).then(() => {
                this.$router.push('/everyExam/everyExamList')
              })
            } else {
              this.$message.error(re.message)
            }
  ...
  rangeTypeChange(value) {
     if (value === 2) {
        this.form.examPaperApplySelect = null
      }
    },
}
</script>
