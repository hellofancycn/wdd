<template>
  <div class="app-container">
    <el-form :model="form" ref="form" label-width="100px">
      <el-form-item label="人员类别结构：" required>
        <el-tree :data="departmentTree" class="wdd-tree" node-key="id"
                 default-expand-all :expand-on-click-node="false" :draggable="treeDraggable"
                 @node-drop="treeNodeDrop">
          <template #default="{ node, data }">
            <span class="customer-tree-node">
              <span>{{ node.label }}</span>
              <span class="tree-action">
                <el-button size="small" @click="() => append(data)" v-has-perm="['department:create']">
                     <el-icon class="button-icon">
                       <circle-plus/>
                     </el-icon>
                     添加
                  </el-button>
                <el-button size="small" @click="() => update(data)" v-has-perm="['department:update']">
                    <el-icon class="button-icon">
                       <icon-edit/>
                     </el-icon>
                     修改
                </el-button>
                <el-button size="small" @click="() => remove(node, data)" v-has-perm="['department:delete']">
                   <el-icon class="button-icon">
                       <icon-delete/>
                   </el-icon>删除
                </el-button>
      ...
      <el-form-item>
        <el-button type="success" @click="addRoot" v-has-perm="['department:create']">添加根节点</el-button>
      </el-form-item>
    </el-form>
    ...
    <el-dialog v-model="dialog.visible" append-to-body :close-on-click-modal="false" :show-close="false" center>
      <el-form :model="form" :rules="nameFormRules" ref="nameForm" label-width="120px">
        <el-form-item label="人员类别名称：" prop="name" required>
...
<script>
import {edit, move, allTree, deleteDepartment} from '@/api/department'
...
export default {
  ...
  data() {
    return {
      ...
      nameFormRules: {
        name: [{required: true, message: '人员类别名称不能为空'}, {
          max: 255,
          message: '长度小于255个字符'
        }]
      },
  ...
  methods: {
    ...
    treeNodeDrop(draggingNode, dropNode, dropType, ev) {
      let message = `此操作会将人员类别【${draggingNode.label}】放置到【${dropNode.label}】, 是否继续?`
    ...
    ,
    remove(node, data) {
      this.$confirm('此操作将永久删除该人员类别, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        deleteDepartment(data.id).then(re => {
          this.load()
        })
      })
    }
