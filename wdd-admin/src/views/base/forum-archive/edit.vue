  ...
  <div class="app-container">
    <el-form :model="form" ref="form" label-width="150px">
      <el-form-item label="文章分类：" required>
        <el-tree :data="forumArchiveTree" class="wdd-tree" node-key="id" default-expand-all
          ...
          <template #default="{ node, data }">
            <span class="customer-tree-node">
              ...
              <span class="tree-action">
                <el-button size="small" @click="() => append(data)" v-has-perm="['forum:archive:create']">
                     <el-icon class="button-icon">
                       <circle-plus/>
                     </el-icon>
                     添加
                  </el-button>
                <el-button size="small" @click="() => update(data)" v-has-perm="['forum:archive:update']">
                    <el-icon class="button-icon">
                       <icon-edit/>
                     </el-icon>
                     修改
                </el-button>
                <el-button size="small" @click="() => remove(node, data)" v-has-perm="['forum:archive:delete']">
                   <el-icon class="button-icon">
                       <icon-delete/>
                     </el-icon>删除
                </el-button>
      ...
      <el-form-item>
        <el-button type="success" @click="addRoot" v-has-perm="['forum:archive:create']">添加根节点</el-button>
      </el-form-item>
    </el-form>

    <el-dialog v-model="dialog.visible" append-to-body :close-on-click-modal="false" :show-close="false" center>
      <el-form :model="form" :rules="nameFormRules" ref="nameForm" label-width="150px">
        <el-form-item label="文章分类名称：" prop="name" required>
...
import {edit, move, tree, deleteArchive} from '@/api/forumArchive'
...
export default {
  ...
  data() {
    return {
      ...
      nameFormRules: {
        name: [{required: true, message: '文章分类名称不能为空'}, {
          max: 255,
          message: '长度小于255个字符'
        }]
      },
  ...
  methods: {
    ...
    treeNodeDrop(draggingNode, dropNode, dropType, ev) {
      let message = `此操作会将文章分类【${draggingNode.label}】放置到【${dropNode.label}】, 是否继续?`
