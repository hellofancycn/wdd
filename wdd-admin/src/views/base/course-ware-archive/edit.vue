<template>
  <div class="app-container">
    <el-form :model="form" ref="form" label-width="150px">
      <el-form-item label="课程分类结构：" required>
        <el-tree :data="courseWareArchiveTree" class="wdd-tree" node-key="id" default-expand-all
          ...
          <template #default="{ node, data }">
            <span class="customer-tree-node">
              ...
              <span class="tree-action">
                <el-button size="small" @click="() => append(data)" v-has-perm="['course:ware:archive:create']">
                     <el-icon class="button-icon">
                       <circle-plus/>
                     </el-icon>
                     添加
                  </el-button>
                <el-button size="small" @click="() => update(data)" v-has-perm="['course:ware:archive:update']">
                    <el-icon class="button-icon">
                       <icon-edit/>
                     </el-icon>
                     修改
                </el-button>
                <el-button size="small" @click="() => remove(node, data)" v-has-perm="['course:ware:archive:delete']">
                   <el-icon class="button-icon">
                       <icon-delete/>
                     </el-icon>删除
                </el-button>
      ...
      <el-form-item>
        <el-button type="success" @click="addRoot" v-has-perm="['course:ware:archive:create']">添加根节点
        </el-button>
      </el-form-item>
    </el-form>
...
import {edit, move, tree, deleteArchive} from '@/api/courseWareArchive'
