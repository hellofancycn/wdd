    ...
    <el-form :model="queryParam" ref="queryForm" :inline="true">
      ...
      <el-form-item>
        <el-button
          type="primary"
          @click="submitForm"
          v-has-perm="['base:dict:page']"
          >查询</el-button
        >
        <el-button @click="clearForm">重置</el-button>
        <router-link :to="{ path: '/base/dataDict/edit' }" class="link-left">
          <el-button type="primary" v-has-perm="['base:dict:create']"
            >添加</el-button
          >
        </router-link>
      </el-form-item>
    ...
    >
      ...
      <el-table-column width="250px" label="操作" align="center">
        <template #default="{ row }">
          <router-link
            :to="{ path: '/base/dataDict/edit', query: { dictId: row.dictId } }"
            class="link-left"
          >
            <el-button
  ...
  methods: {
    ...
    examPaperDelete(row) {
      this.$confirm('将执行删除操作, 是否继续?', '提示', {
        ...
        deleteDictApi({ dictIds: [row.dictId] }).then(re => {
          if (re.code === 1) {
            this.search();
